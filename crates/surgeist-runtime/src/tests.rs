use super::testing::concurrency::{BlockingWakeBridge, ScenarioDeadline, run_in_child};
use super::testing::{FakeWakeBridge, HeadlessHarness, PrototypeApp, PrototypeServiceEvent};
use super::*;
...
fn app_proxy_waiter_resignals_after_failed_owner_wake_while_drain_waits() {
    run_in_child(
        "tests::app_proxy_waiter_resignals_after_failed_owner_wake_while_drain_waits",
        || {
            ...
            let first_wake = controller.next(1).unwrap();
            ...
            deadline
                .wait_for("concurrent sender enqueue", 2, || proxy.pending_len())
                .unwrap();
            deadline
                .wait_for("concurrent sender wait registration", 1, || {
            ...
            deadline
                .wait_for("racing drain condition-variable wait", 1, || {
                    proxy.waiting_drain_count()
                })
                .unwrap();

            first_wake
                .release(Err(WakeError::new("first wake failed")))
                .unwrap();
            let owner_error = deadline
                .receive("owner completion", &owner_done_rx)
                .unwrap()
                .unwrap_err();
            ...
            let second_wake = controller.next(2).unwrap();
            second_wake.release(Ok(())).unwrap();
            assert!(
                deadline
                    .receive("waiter completion", &waiter_done_rx)
                    .unwrap()
                    .is_ok()
            );
            deadline.join("owner thread", owner).unwrap();
            deadline.join("waiter thread", waiter).unwrap();

            assert_eq!(
                deadline
                    .receive("drain completion", &drain_done_rx)
                    .unwrap()
                    .into_drained(),
                vec![ProxyInput::Task(waiter_input)]
            );
            deadline.join("drainer thread", drainer).unwrap();
            assert_eq!(proxy.pending_len(), 0);
        },
    );
}
...
fn app_proxy_partial_drain_successor_wake_covers_racing_sender() {
    run_in_child(
        "tests::app_proxy_partial_drain_successor_wake_covers_racing_sender",
        || {
            let deadline = ScenarioDeadline::new();
            let (wake, controller) = BlockingWakeBridge::new(&deadline);
            let proxy = Arc::new(AppProxy::<CounterInput>::new(wake, QueuePolicy::bounded(4)));
            ...
            let first_wake = controller.next(1).unwrap();
            first_wake.release(Ok(())).unwrap();
            assert!(
                deadline
                    .receive("first completion", &first_done_rx)
                    .unwrap()
                    .is_ok()
            );
            deadline.join("first_sender thread", first_sender).unwrap();
            ...
            deadline
                .wait_for("racing sender enqueue behind drain wake", 2, || {
                    proxy.pending_len()
                })
                .unwrap();
            deadline
                .wait_for("racing sender wait registration", 1, || {
                    proxy.waiting_sender_count()
                })
                .unwrap();
            ...
            assert!(
                deadline
                    .receive("racing completion", &racing_done_rx)
                    .unwrap()
                    .is_ok()
            );
            deadline.join("drainer thread", drainer).unwrap();
            deadline
                .join("racing_sender thread", racing_sender)
                .unwrap();
            assert_eq!(proxy.pending_len(), 2);
...
fn app_proxy_partial_drain_failure_waiter_resignals_without_stranding_inputs() {
    run_in_child(
        "tests::app_proxy_partial_drain_failure_waiter_resignals_without_stranding_inputs",
        || {
            let deadline = ScenarioDeadline::new();
            let (wake, controller) = BlockingWakeBridge::new(&deadline);
            let proxy = Arc::new(AppProxy::<CounterInput>::new(wake, QueuePolicy::bounded(4)));
            ...
            deadline
                .wait_for("racing sender enqueue behind failed drain wake", 2, || {
                    proxy.pending_len()
                })
                .unwrap();
            deadline
            ...
            second_wake
                .release(Err(WakeError::new("continuation failed")))
                .unwrap();
            let report = deadline
                .receive("drain completion", &drain_done_rx)
                .unwrap();
            ...
            let third_wake = controller.next(3).unwrap();
            third_wake.release(Ok(())).unwrap();
            assert!(
                deadline
                    .receive("racing completion", &racing_done_rx)
                    .unwrap()
...
#[test]
fn effect_kind_ids_preserve_declared_runtime_names() {
    let backed = [
        EffectKindId::REQUEST_REDRAW,
...
fn runtime_intents_preserve_each_owned_effect_payload() {
    ...
    let handle = TaskIntentHandle::new(TaskIntentId::from_u64(7), TaskIntentAttemptId::from_u64(2));
    let service = ServiceId::new("jsonrpc");
    let provenance = InputProvenance::system().with_sequence(73);
    let effects = [
    ...
    let batch = effects
        .into_iter()
        .fold(EffectBatch::new(), EffectBatch::push);
    let mut runtime = Runtime::new(
        CounterState::default(),
        EffectsReducer {
            commit: ReducerCommit::new().with_effects(batch),
        },
    );
    runtime
        .enqueue_ui(UiInput::new(CounterInput::RedrawAll, provenance.clone()).unwrap())
        .unwrap();
    ...
    assert_eq!(report.rejected_effects(), 0);
    assert!(runtime.diagnostics().entries().is_empty());
    let [
        RuntimeIntent::Persist(persist),
        RuntimeIntent::LoadResource(load),
        RuntimeIntent::InvalidateResource(invalidate),
        RuntimeIntent::StartTask(start),
        RuntimeIntent::CancelTask(cancel),
        RuntimeIntent::ReprioritizeTask(reprioritize),
        RuntimeIntent::StartService(start_service),
        RuntimeIntent::StopService(stop_service),
        RuntimeIntent::CallService(call),
    ] = report.intents()
    else {
        panic!("forwarded intents must preserve all nine effect kinds and their order");
    };

    assert_eq!(persist.key(), "session");
    assert_eq!(persist.scope(), &scope);
    assert_eq!(load.operation(), &operation);
    assert_eq!(load.id(), &ResourceId::new("thumb:1"));
    assert_eq!(load.scope(), &scope);
    assert_eq!(invalidate.id(), &ResourceId::new("thumb:1"));
    assert_eq!(invalidate.reason(), "source changed");
    assert_eq!(start.name(), &TaskIntentName::new("search"));
    assert_eq!(start.key(), &TaskIntentKey::new("search:rust"));
    assert_eq!(start.scope(), &scope);
    assert_eq!(cancel.handle(), handle);
    assert_eq!(reprioritize.handle(), handle);
    assert_eq!(reprioritize.priority(), TaskPriorityHint::High);
    assert_eq!(start_service.id(), &service);
    assert_eq!(stop_service.id(), &service);
    assert_eq!(call.id(), &service);
    assert_eq!(call.command().as_str(), "textDocument/hover");
    assert_eq!(call.payload().as_json_text(), r#"{"line":3}"#);
    assert_eq!(call.correlation(), correlation(42));
    ...
    assert_eq!(report.effect_outcomes().len(), kinds.len());
    for ((outcome, kind), intent) in report
        .effect_outcomes()
        .iter()
        .zip(kinds)
        .zip(report.intents())
    {
        ...
        assert_eq!(outcome.disposition(), EffectDisposition::Forwarded);
        assert_eq!(outcome.provenance(), &provenance);
        assert_eq!(outcome.intent(), Some(intent));
...
#[test]
fn prototype_log_stream_preserves_every_entry_in_order_across_budgeted_turns() {
    let mut app = PrototypeApp::log_stream(RuntimeBudget::new(10, 0, 10, 0));
    let expected = (0..35)
    ...
    assert_eq!(app.log_lines(), &expected[..10]);
    assert_eq!(first.drained_inputs(), 10);
    assert_eq!(first.remaining_task_inputs(), 25);
    assert!(first.has_pending_inputs());

    let mut reports = vec![first];
    reports.extend(app.drain_all(NonZeroUsize::new(3).unwrap()).unwrap());

    assert_eq!(app.log_lines(), expected);
    assert_eq!(
        reports
            .iter()
            .map(RuntimeDrainReport::drained_inputs)
            .collect::<Vec<_>>(),
        [10, 10, 10, 5]
    );
    for (report, remaining) in reports.iter().zip([25, 15, 5, 0]) {
        assert_eq!(report.remaining_ui_inputs(), 0);
        assert_eq!(report.remaining_task_inputs(), remaining);
        assert_eq!(report.remaining_service_inputs(), 0);
        assert_eq!(report.has_pending_inputs(), remaining != 0);
    }
    assert_eq!(app.proxy().pending_len(), 0);
}

#[test]
fn ten_thousand_task_events_use_one_wake_and_preserve_every_index_within_budget() {
    let mut app = PrototypeApp::progress_counter(RuntimeBudget::new(128, 0, 128, 0));
    let expected = (0..10_000).collect::<Vec<_>>();

    for index in &expected {
        app.proxy().send_task(app.progress_event(*index)).unwrap();
    }

    assert!(app.progress_indices().is_empty());
    assert_eq!(app.proxy().pending_len(), 10_000);
    assert_eq!(app.fake_wake().wake_count(), 1);

    let reports = app.drain_all(NonZeroUsize::new(79).unwrap()).unwrap();

    assert_eq!(app.progress_indices(), expected);
    let mut expected_turn_sizes = vec![128; 78];
    expected_turn_sizes.push(16);
    assert_eq!(
        reports
            .iter()
            .map(RuntimeDrainReport::drained_inputs)
    ...
    let mut expected_remaining = 10_000;
    for (report, turn_size) in reports.iter().zip(expected_turn_sizes) {
        expected_remaining -= turn_size;
        assert_eq!(report.first_drained_lane(), Some(RuntimeLane::Task));
        assert_eq!(report.remaining_ui_inputs(), 0);
        assert_eq!(report.remaining_task_inputs(), expected_remaining);
        assert_eq!(report.remaining_service_inputs(), 0);
        assert_eq!(report.has_pending_inputs(), expected_remaining != 0);
    }
    assert_eq!(app.fake_wake().wake_count(), 1);
    assert_eq!(app.proxy().pending_len(), 0);
}
...
#[test]
fn service_queue_preserves_each_event_and_provenance_in_two_input_batches() {
    let mut app = PrototypeApp::service_recorder(RuntimeBudget::new(2, 0, 0, 2));
    let service = ServiceId::new("jsonrpc");
    let expected = [
        AppInput::new(
            PrototypeServiceEvent::Progress {
                request: 202,
                message: "half".to_owned(),
            },
            InputProvenance::service(service.clone())
                .with_correlation(correlation(202))
                .with_sequence(1),
        ),
        AppInput::new(
            PrototypeServiceEvent::Response {
                request: 101,
                message: "compiled".to_owned(),
            },
            InputProvenance::service(service.clone())
                .with_correlation(correlation(101))
                .with_sequence(2),
        ),
        AppInput::new(
            PrototypeServiceEvent::Cancelled { request: 202 },
            InputProvenance::service(service.clone())
                .with_correlation(correlation(202))
                .with_sequence(3),
        ),
        AppInput::new(
            PrototypeServiceEvent::TimedOut { request: 303 },
            InputProvenance::service(service.clone())
                .with_correlation(correlation(303))
                .with_sequence(4),
        ),
        AppInput::new(
            PrototypeServiceEvent::Reconnected,
            InputProvenance::service(service).with_sequence(5),
        ),
    ];

    assert_eq!(app.fake_wake().wake_count(), 0);
    assert_eq!(app.proxy().pending_len(), 0);
    for input in &expected {
        app.send_service(input.payload().clone(), input.provenance().clone());
    }
    assert!(app.service_events().is_empty());
    assert_eq!(app.fake_wake().wake_count(), 1);
    assert_eq!(app.proxy().pending_len(), 5);

    for (drained, delivered, remaining) in [(2, 2, 3), (2, 4, 1), (1, 5, 0)] {
        let report = app.drain();

        assert_eq!(report.drained_inputs(), drained);
        assert_eq!(report.first_drained_lane(), Some(RuntimeLane::Service));
        assert_eq!(report.remaining_ui_inputs(), 0);
        assert_eq!(report.remaining_task_inputs(), 0);
        assert_eq!(report.remaining_service_inputs(), remaining);
        assert_eq!(report.has_pending_inputs(), remaining != 0);
        assert_eq!(app.service_events(), &expected[..delivered]);
        assert_eq!(app.proxy().pending_len(), 0);
    }
    assert_eq!(app.service_events(), expected);
}
