...
originating in
[the historical R06 plan](https://github.com/bj-data-eng/surgeist-layout/blob/20ad8202e536c4c63f0bd211f0872653462116bf/plans/cycles/P01-I08-S02-R06-node-projections-and-compatible-api-map.md)
at published revision `20ad8202e536c4c63f0bd211f0872653462116bf`. The old
planning directory is not part of the snapshot import. That R06 plan was governed by
...
- Setup is part of the audit's semantics, not incidental bootstrap detail. The
  ...
  dynamic library. The catalog remains a separate Cargo workspace with its own lockfile and
  nightly toolchain. Every catalog build and test uses an explicitly selected
  `CARGO_TARGET_DIR`; the historical layout-local convention is
  `crates/surgeist-layout/target/dylint-audits` from the product root. Keep one
  target directory for library discovery and cleanup, and run from this tool
  directory so its `.cargo/config.toml` applies.
...
- These are crate-local pilot lessons only. Turning them into reusable shared
