...
#[cfg(test)]
#[test]
fn media_query_original_adapters_admit_independent_classes_and_diagnostics() {
    // Independent pinned grammar/diagnostic audit, not the committed class registry
    // or captured oracle. Exercise the real adapter -> observer -> class gate.
    ...
    for row in rows {
        ...
        let record = observe_csstree_record(case)
            .unwrap_or_else(|error| panic!("{id}: real adapter/class admission failed: {error:?}"));
        let observed = serde_json::to_value(record).unwrap();
        assert_eq!(observed["probe"]["entry_point"], "media_query", "{id}");
        assert_eq!(observed["probe"]["payload"]["prefix"], "", "{id}");
        assert_eq!(observed["probe"]["payload"]["suffix"], "", "{id}");
        assert_eq!(
            observed["probe"]["payload"]["input_byte_length"],
        ...
        assert_eq!(
            observed["observation"]["extractor"],
            serde_json::json!({"kind":"media_query"}),
            "{id}"
        );
        assert_eq!(observed["observation"]["syntax_count"], 1, "{id}");
        assert_eq!(observed["observation"]["is_clean"], row["clean"], "{id}");
        assert_eq!(
            observed["observation"]["diagnostics"], row["diagnostics"],
            "{id}: ordered, multiplicity-one diagnostics"
...
#[cfg(test)]
#[test]
fn stylesheet_original_adapters_admit_independent_classes_and_diagnostics() {
    // Independent pinned grammar/diagnostic audit, not the committed class registry
    // or captured oracle. Exercise the real adapter -> observer -> class gate.
    let data: serde_json::Value =
    ...
    for row in rows {
        ...
        let observed = serde_json::to_value(record).unwrap();
        assert_eq!(observed["probe"]["entry_point"], "sheet", "{id}");
        assert_eq!(observed["probe"]["payload"]["prefix"], "", "{id}");
        assert_eq!(observed["probe"]["payload"]["suffix"], "", "{id}");
        assert_eq!(
            observed["probe"]["payload"]["input_byte_length"],
        ...
        assert_eq!(
            observed["observation"]["extractor"],
            serde_json::json!({"kind":"sheet_rules"}),
            "{id}"
        );
        assert_eq!(
            observed["observation"]["syntax_count"], row["expected"]["top_level_rule_count"],
            "{id}"
        );
        assert_eq!(
...
#[cfg(test)]
#[test]
fn remaining_pseudo_original_adapters_admit_independent_classes_and_diagnostics() {
    // Independent pinned grammar/diagnostic audit, not the committed class registry
    // or captured oracle. Exercise the real adapter -> observer -> class gate.
    let data: serde_json::Value = serde_json::from_str(include_str!(
        "../csstree/remaining-pseudo-reconciliation.json"
    ))
    .unwrap();
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join(CORPUS_ROOT);
    let mut inventory = read_neutral_artifact_set(&root)
...
#[cfg(test)]
#[test]
fn declaration_list_original_adapters_admit_independent_raw_classes_and_diagnostics() {
    // Independent pinned grammar/diagnostic audit, not the committed class registry
    // or captured oracle. Exercise the real adapter -> observer -> class gate.
    let data: serde_json::Value = serde_json::from_str(include_str!(
        "../csstree/declaration-list-reconciliation.json"
    ))
    .unwrap();
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join(CORPUS_ROOT);
    let mut inventory = read_neutral_artifact_set(&root)
    ...
    for row in rows {
        ...
        let binding = REGISTRY
            ...
            .copied()
            .unwrap();
        let binding = resolve_case_registry(case, binding).unwrap();
        let complete = binding
            .adapter()
            .wrap(&case.input, binding.property_or_descriptor())
            .unwrap();
        let span = complete.payload_span();
        let actual_adapter = serde_json::json!({"entry_point": binding.entry_point_name(),
            "prefix": &complete.source()[..span.start], "suffix": &complete.source()[span.end..],
            "extractor": raw_extractor(binding.extractor())});
        let expected_adapter = serde_json::json!({"entry_point": "style_attribute", "prefix": "", "suffix": "",
            "extractor": {"kind": "declaration_list"}});
        if actual_adapter != expected_adapter {
            failures.push(format!("{id}: original declaration-list grammar requires raw adapter\nexpected: {expected_adapter}\nactual: {actual_adapter}"));
        }
        if let Err(error) = validate_expected_class(case, case.expected_class.as_ref().unwrap()) {
            failures.push(format!(
                "{id}: independent raw class admission failed: {error}"
            ));
            continue;
        }
        let record = match observe_csstree_record(case) {
        ...
        let observed = serde_json::to_value(record).unwrap();
        assert_eq!(observed["probe"]["entry_point"], "style_attribute", "{id}");
        assert_eq!(observed["probe"]["payload"]["prefix"], "", "{id}");
        assert_eq!(observed["probe"]["payload"]["suffix"], "", "{id}");
        assert_eq!(
            observed["probe"]["payload"]["input_byte_length"],
        ...
        assert_eq!(
            observed["observation"]["extractor"],
            serde_json::json!({"kind":"declaration_list"}),
            "{id}"
        );
        assert_eq!(
            observed["observation"]["syntax_count"], row["expected"]["declaration_count"],
            "{id}"
        );
        assert_eq!(
            observed["observation"]["is_clean"], row["expected"]["is_clean"],
...
#[cfg(test)]
#[test]
fn selector_original_adapters_admit_independent_classes_and_diagnostics() {
    // Independent pinned grammar/diagnostic audit, not the committed class registry
    // or captured oracle. Exercise the real adapter -> observer -> class gate.
    let data: serde_json::Value = serde_json::from_str(include_str!(
