...
Container queries retain authored grouping and operator order on `@container` group rules. Parsed and checked conditions share immutable component syntax, which also supplies their serialization and source origins. Read the condition kind through its accessor and construct conditions through checked component admission. Container matching and mutable CSSOM behavior belong to downstream Surgeist layers.
