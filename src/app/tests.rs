...
fn provenance_carries_causal_fields() {
    ...
    assert_eq!(child.sequence(), Some(6));

    let service = InputProvenance::service(ServiceId::new("thumb-loader"));
    assert_eq!(service.source(), &InputSourceId::SERVICE);
    assert_eq!(service.service_id(), Some(&ServiceId::new("thumb-loader")));
}

#[test]
fn provenance_accepts_custom_input_source_ids() {
    let provenance = InputProvenance::new(InputSourceId::new("plugin.camera"))
        .with_correlation(CorrelationId::from_u64(42));

    assert_eq!(provenance.source().as_str(), "plugin.camera");
    assert_eq!(provenance.correlation_id(), CorrelationId::from_u64(42));
}
...
fn diagnostics_keep_recent_entries_and_counters() {
    ...
    let borrowed_entries = log.entry_refs().collect::<Vec<_>>();
    let entries = log.entries();
    assert_eq!(borrowed_entries.len(), 2);
    ...
    assert_eq!(borrowed_entries[0].severity(), DiagnosticSeverity::Error);
    assert_eq!(
        borrowed_entries[0].code(),
        &DiagnosticCode::STALE_TASK_EVENT
    );
    assert_eq!(borrowed_entries[0].app_id(), Some(&AppId::new("photo.lab")));
...
fn diagnostic_log_capacity_zero_retains_no_entries_but_counts_inputs() {
    ...
    assert_eq!(log.entry_refs().count(), 0);
    assert_eq!(log.dropped_oldest(), 2);
    assert_eq!(log.count(&DiagnosticCode::QUEUE_OVERFLOW), 2);
}
...
fn reducer_returns_effects_without_executing_them() {
    ...
    let save_result = reducer.reduce(
        &mut state,
        AppInput::new(CounterInput::Save, InputProvenance::system()),
    );

    assert!(!save_result.is_changed());
    assert_eq!(save_result.effects().len(), 1);
    assert_eq!(save_result.effects()[0].kind(), &EffectKindId::PERSIST);
}

#[test]
fn reducer_result_exposes_recoverable_error_message() {
    let result = ReducerResult::recoverable_failure("counter rejected input");

    assert!(!result.is_changed());
    assert_eq!(result.recoverable_error(), Some("counter rejected input"));
}
