...
fn extended_sheets_preserve_manifest_load_order() {
    let mut sheet = css::parse_sheet(".panel { color: #111111; }").unwrap();
    let theme = css::parse_sheet(".panel { color: #222222; }").unwrap();
...
#[test]
fn rejects_style_domain_errors_during_css_parsing() {
    let error = css::parse_sheet(".panel { opacity: 2; }").unwrap_err();

    assert!(
        error.message().contains("Opacity must be between 0 and 1"),
        "{error}"
    );
}
